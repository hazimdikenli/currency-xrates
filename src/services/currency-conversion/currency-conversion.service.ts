...
export class CurrencyConversionService {
  ...
  private doConversion(
    ...
    fromCC: CurrencyCode,
    toCC: CurrencyCode,
    xrates: CurrencyExchangeRateDto[]
  ): number {
    const exactMatch = xrates.find(
      f => f.currency_code === fromCC && f.target_currency_code === toCC
    )

    if (exactMatch) {
      return (amount / exactMatch.amount) * Number(exactMatch.target_amount)
    }
    const reverseMatch = xrates.find(
      f => f.currency_code === toCC && f.target_currency_code === fromCC
    )
    if (reverseMatch) {
      return (amount * reverseMatch.amount) / Number(reverseMatch.target_amount)
    }
    // try match through common currency
    const sourceMatch = xrates.filter(
      f => f.currency_code === fromCC || f.target_currency_code === fromCC
    )
    const targetMatch = xrates.filter(
      f => f.currency_code === toCC || f.target_currency_code === toCC
    )
    const commonCurrenciesSet = new Set(
      sourceMatch
        .flatMap(f => [f.currency_code, f.target_currency_code])
        .concat(targetMatch.flatMap(f => [f.currency_code, f.target_currency_code]))
    )
