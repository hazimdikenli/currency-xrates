...
export class KznbXRateService {
  constructor(
    ...
    private ratesDownloader: KznbXRatesDownloader
  ) {}
  ...
  async downloadDailyRates(dateToDownload: string) {
    ...
    if (downloadedDate !== dateToDownload)
      throw new Error(`Invalid date: requested ${dateToDownload} , received: ${downloadedDate}`)

    const rates: CurrencyExchangeRateCreate[] = exchangeRates.map(
      ({ rate, currency_code, amount }) => ({
        amount,
        currency_code,
        target_amount: rate,
